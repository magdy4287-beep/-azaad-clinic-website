...
text = re.sub(
    r'const SUPABASE_PUBLISHABLE_KEY = "([^"]+)";\s*\n\s*const SUPABASE_AUTH_STORAGE_KEY = "[^"]+";',
    r'const SUPABASE_PUBLISHABLE_KEY = "\1";\n\nconst SUPABASE_AUTH_STORAGE_KEY = "' + CANONICAL_STORAGE_KEY + '";',
    text,
    count=1,
)
text = re.sub(
    r"auth:\s*\{\s*\n\s*persistSession:\s*true,\s*\n\s*autoRefreshToken:\s*true,\s*\n\s*detectSessionInUrl:\s*true,\s*\n\s*storage:\s*window\.localStorage,\s*\n\s*storageKey:\s*SUPABASE_AUTH_STORAGE_KEY\s*\n\s*\}",
    "auth: {\n      persistSession: true,\n      autoRefreshToken: true,\n      detectSessionInUrl: false,\n      storage: window.localStorage,\n      storageKey: SUPABASE_AUTH_STORAGE_KEY\n    }",
    text,
    count=1,
)
pattern = re.compile(r"async function restoreStaffProfile\(\)\s*\{.*?\n\}\n\n/\* ============================================================\n   INITIALIZE", re.S)
replacement = '''async function restoreStaffProfile() {
  try {
    ...
    const request = async (token) => fetch(`${SUPABASE_URL}/functions/v1/azaad-admin-auth`, {
      ...
      headers: { Accept: "application/json", Authorization: `Bearer ${token}`, apikey: SUPABASE_PUBLISHABLE_KEY }
    });
    ...
    if (session?.refresh_token) {
      try { await supabase.auth.setSession({ access_token: accessToken, refresh_token: session.refresh_token }); } catch (_) {}
    }
    return true;
  } catch (error) {
...
print("Unified admin auth lifecycle: canonical persistent storage + reload-safe server authorization")

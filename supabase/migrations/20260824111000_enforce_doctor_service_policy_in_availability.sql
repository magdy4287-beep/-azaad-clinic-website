...
begin
  ...
  if not found then return; end if;
  if not public.clinic_doctor_service_policy_allows(p_doctor_id,p_service_id) then return; end if;
  if array_length(v_doctor.services,1) is not null and not (p_service_id::text = any(v_doctor.services::text[])) then return; end if;
  select * into v_override from public.doctor_schedule_overrides where doctor_id=p_doctor_id and override_date=p_appointment_date order by updated_at desc nulls last limit 1;
  if found and lower(coalesce(v_override.type,'')) in ('closed','off','unavailable','holiday') then return; end if;
  select * into v_holiday from public.clinic_holidays where p_appointment_date between start_date and end_date and closed=true and (applies_to='all' or applies_to='clinic' or (applies_to='doctor' and doctor_id=p_doctor_id)) limit 1;
  if found then return; end if;
  select * into v_schedule from public.doctor_weekly_schedules where doctor_id=p_doctor_id and weekday=extract(dow from p_appointment_date)::integer and enabled=true and (mode=v_mode or mode='both' or (v_mode='clinic' and mode='in_clinic')) order by case when mode=v_mode then 0 when mode='both' then 1 else 2 end limit 1;
  if not found then return; end if;
  ...
  if v_max_daily is not null then
    select count(*) into v_daily_count from public.clinic_bookings where doctor_id=p_doctor_id and appointment_date=p_appointment_date and status in ('pending','confirmed');

-- Production hotfix mirrored from the live database migration.
-- PostgreSQL regex patterns must use a single backslash in the SQL string.
CREATE OR REPLACE FUNCTION private.clinic_update_patient_demographics(
  p_patient_id uuid,p_patient_name text,p_patient_phone text,p_patient_email text,
  p_date_of_birth date,p_height_cm numeric,p_weight_kg numeric,
  p_gender text DEFAULT NULL,p_marital_status text DEFAULT NULL,p_residence text DEFAULT NULL
)
...
begin
  ...
  if v_role='DOCTOR' and not exists (select 1 from public.clinic_bookings b where b.patient_id=p_patient_id and b.doctor_id=v_staff.doctor_id union all select 1 from public.clinic_clinical_visits v where v.patient_id=p_patient_id and v.doctor_id=v_staff.doctor_id) then raise exception 'PATIENT_EDIT_SCOPE_DENIED'; end if;

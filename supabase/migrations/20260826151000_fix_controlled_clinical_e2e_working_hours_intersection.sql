-- Keep controlled clinical E2E slot discovery inside the clinic's actual working hours.
-- Doctor schedules may extend beyond clinic hours; availability is their intersection.
...
begin
  ...
  with candidates as (
    select current_date+gs.day_offset candidate_date,
           (greatest(effective.start_time,wh.start_time)+(slot.n*interval '30 minutes'))::time candidate_time,
           greatest(effective.start_time,wh.start_time) start_time,
           least(effective.end_time,wh.end_time) end_time,
           effective.break_start,effective.break_end
  ...
  where c.candidate_time>=c.start_time and c.candidate_time+make_interval(mins=>v_duration_minutes)<=c.end_time

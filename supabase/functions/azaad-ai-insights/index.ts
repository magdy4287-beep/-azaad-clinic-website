...
async function auth(req: Request) {
  ...
  const s = await db.from("clinic_staff").select("id,role,active,account_status").eq("auth_user_id", u.data.user.id).eq("active", true).maybeSingle();
  if (!s.data?.active || s.data.account_status !== "active") return null;
...
const reviewRoles = new Set(["OWNER", "ADMIN", "MANAGER"]);
...
Deno.serve(async req => {
  ...
  if (req.method === "POST") {
    if (!reviewRoles.has(String(staff.role).toUpperCase())) return json({ error: "Insight review requires OWNER, ADMIN or MANAGER" }, 403);
    try {
      const body = await req.json();
      const insightId = String(body?.insight_id || "");
      const reviewStatus = String(body?.review_status || "").toUpperCase();
      const reviewNote = body?.review_note == null ? null : String(body.review_note).slice(0, 2000);
      if (!insightId || !["APPROVED", "REJECTED", "PENDING"].includes(reviewStatus)) return json({ error: "insight_id and valid review_status are required" }, 400);
      const { data, error } = await db.from("clinic_ai_insights")
        .update({ review_status: reviewStatus, reviewed_by: reviewStatus === "PENDING" ? null : staff.id, reviewed_at: reviewStatus === "PENDING" ? null : new Date().toISOString(), review_note: reviewNote })
        .eq("id", insightId)
        .select("id,review_status,reviewed_by,reviewed_at,review_note")
        .maybeSingle();
      if (error) return json({ error: error.message }, 500);
      if (!data) return json({ error: "Insight not found" }, 404);
      return json({ ok: true, review: data });
    } catch (error) {
      return json({ error: error instanceof Error ? error.message : "Invalid review request" }, 400);
    }
  }
  ...
  const kpis = {
    bookings: bookings.length, completed, no_show: noShow, cancelled,
    ...
    invoiced, collected, outstanding: Math.max(invoiced - collected, 0), expenses: spent,
    net_cash_flow: collected - spent, open_followups: followups.length,
    overdue_followups: overdueFollowups, open_alerts: alerts.length, active_staff: staffRows.length,
  };
  ...
  const add = (type: string, severity: string, ar: string, en: string, recAr: string, recEn: string, value: number | null = null, unit: string | null = null, payload: any = {}) => insights.push({
    scope: "management", insight_type: type, severity, title_ar: ar, title_en: en, summary_ar: ar, summary_en: en,
    recommendation_ar: recAr, recommendation_en: recEn, metric_value: value, metric_unit: unit, payload,
  });
  ...
  if (reviewRoles.has(String(staff.role).toUpperCase())) {
    await db.from("clinic_ai_insights").update({ status: "EXPIRED" }).eq("scope", "management").eq("status", "OPEN").lt("expires_at", new Date().toISOString());
    const { data: existing, error: existingError } = await db.from("clinic_ai_insights")
      .select("id,insight_type,review_status,reviewed_by,reviewed_at,review_note")
      .eq("scope", "management").eq("status", "OPEN")
      .gte("generated_at", `${from}T00:00:00`).lte("generated_at", `${to}T23:59:59.999`);
    ...
    for (const x of insights) {
      ...
      const { error } = await db.from("clinic_ai_insights").insert({ ...x, status: "OPEN", review_status: "PENDING", generated_at: new Date().toISOString(), expires_at: expires });
      if (error) return json({ error: error.message }, 500);
    }
  }

  const { data: current, error: currentError } = await db.from("clinic_ai_insights")
    .select("id,scope,insight_type,severity,title_ar,title_en,summary_ar,summary_en,recommendation_ar,recommendation_en,metric_value,metric_unit,payload,status,generated_at,expires_at,review_status,reviewed_by,reviewed_at,review_note")
    .eq("scope", "management").eq("status", "OPEN")
    .gte("generated_at", `${from}T00:00:00`).lte("generated_at", `${to}T23:59:59.999`)
    .order("generated_at", { ascending: false });
  if (currentError) return json({ error: currentError.message }, 500);
  return json({ ok: true, provider: "free-local-rules-engine", from, to, kpis, insights: current || [], generated_insights: insights });
});

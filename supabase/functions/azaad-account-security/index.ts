...
async function getActor(token: string) {
  if (!token) return null;
  const { data, error } = await authClient.auth.getUser(token);
  if (error || !data.user) return null;
  const { data: staff } = await adminClient
    .from("clinic_staff")
    .select("id,auth_user_id,username,full_name,role,phone,active,account_status")
    .eq("auth_user_id", data.user.id)
    .maybeSingle();
  if (!staff || staff.active === false || staff.account_status !== "active") return null;
  return { user: data.user, staff };
}

async function ownerOnly(token: string) {
  const actor = await getActor(token);
  if (!actor || String(actor.staff.role).toUpperCase() !== "OWNER") return null;
  return actor;
}

async function audit(staffId: string, actorId: string | null, action: string, reason?: string, metadata: Record<string, unknown> = {}) {
  await adminClient.from("clinic_account_security_audit").insert({
    staff_id: staffId,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "request_otp") {
      ...
      const { data: staff } = await adminClient
        .from("clinic_staff")
        .select("id,auth_user_id,username,phone,active,account_status")
        .eq("username", username)
        .maybeSingle();

      // Deliberately generic to reduce account enumeration.
      if (!staff || !staff.auth_user_id || staff.active === false || staff.account_status !== "active" || normalizePhone(staff.phone || "") !== phone) {
        return json({ ok: true, message: "إذا كانت البيانات صحيحة فسيتم إرسال رمز التحقق." });
      }

      const { data: authUser } = await adminClient.auth.admin.getUserById(staff.auth_user_id);
      if (!authUser.user?.phone || normalizePhone(authUser.user.phone) !== phone) {
        return json({ error: "PHONE_NOT_BOUND_TO_AUTH" }, 409);
      }
    ...
    if (action === "verify_otp_set_password") {
      ...
      const { error: updateError } = await adminClient.auth.admin.updateUserById(staff.auth_user_id, {
        password,
    ...
    const { data: target } = await adminClient
      .from("clinic_staff")
      .select("id,auth_user_id,username,role,active,account_status")
      .eq("id", staffId)
      .maybeSingle();
    ...
    if (action === "suspend" || action === "disable" || action === "reactivate" || action === "archive") {
      ...
      const update = await adminClient.from("clinic_staff").update({
        account_status: status,
        active: status === "active",
        account_status_changed_at: new Date().toISOString(),
        account_status_changed_by: actor.staff.id,
        terminated_at: ["disabled", "archived"].includes(status) ? new Date().toISOString() : null,
        updated_at: new Date().toISOString(),
      }).eq("id", staffId).select("id,username,full_name,role,phone,active,account_status").single();
      if (update.error) return json({ error: "ACCOUNT_STATUS_UPDATE_FAILED" }, 502);

      if (target.auth_user_id) {
        const banDuration = status === "active" ? "none" : "876000h";
        const authUpdate = await adminClient.auth.admin.updateUserById(target.auth_user_id, { ban_duration: banDuration });
        if (authUpdate.error) return json({ error: "AUTH_STATUS_UPDATE_FAILED" }, 502);
      }

      const auditAction = action === "suspend" ? "suspend" : action === "disable" ? "disable" : action === "archive" ? "archive" : "reactivate";
      await audit(target.id, actor.staff.id, auditAction, String(body?.reason || ""), { previous_status: target.account_status, new_status: status });
    ...
    return json({ error: "SECURITY_OPERATION_FAILED" }, 500);
  }
});

...
Deno.serve(async (req) => {
  ...
  const { data, error } = await supabase.rpc("clinic_frontdesk_checkin", {
    p_booking_id: bookingId,
    p_notes: body.notes ?? null,
  });
  if (error) {
    const message = String(error.message ?? "checkin_failed");
    const status = /NOT_AUTHORIZED|permission_denied/i.test(message) ? 403 : /NOT_FOUND/i.test(message) ? 404 : 409;

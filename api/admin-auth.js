...
import { runtimeContract, jsonResponse } from '../lib/azaad-runtime-contract.js';
...
async function verifyAppwrite() {
  ...
  try {
    const response = await fetch(`${endpoint}/users?limit=1`, {
      method: 'GET',
      headers: { 'X-Appwrite-Project': project, 'X-Appwrite-Key': key, Accept: 'application/json' }
    });
    return { configured: true, reachable: response.ok, usersApi: response.ok };
  } catch (error) {
...
async function runtimeHealth() {
  ...
  const [appwrite, targetFingerprint] = await Promise.all([verifyAppwrite(), databaseTargetFingerprint()]);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'runtime-health') return runtimeHealth();

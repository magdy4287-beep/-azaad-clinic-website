...
async function identity(req, sql) {
  ...
  if (!endpoint || !project || !session) return null;
  const r = await fetch(`${endpoint}/account`, { headers: { 'X-Appwrite-Project': project, Cookie: `a_session_${project}=${session}`, accept: 'application/json' } });
  if (!r.ok) return null;
  ...
  return { staff, role: String(staff.role || '').toUpperCase() };
}
function allowed(role, action) { return ROLES[action]?.has(role) === true; }
...
export default async function handler(req, res) {
  try {
    ...
    const auth = await identity(req, sql);
    if (!auth) return json(res, { error: 'authentication_required' }, 401);
    const url = new URL(req.url, `https://${req.headers.host || 'localhost'}`);
    const action = String(url.searchParams.get('action') || '').toLowerCase();

    if (req.method === 'GET') {
      if (!allowed(auth.role, 'request') && !allowed(auth.role, 'doctor') && !allowed(auth.role, 'management') && !allowed(auth.role, 'process')) return json(res, { error: 'forbidden' }, 403);
      const rows = await sql`
        ...
        order by r.requested_at desc nulls last, r.created_at desc nulls last limit 100
      `;
      return json(res, { provider: 'appwrite-neon', role: auth.role, refunds: rows });
    }
    ...
    if (action === 'request') {
      if (!allowed(auth.role, 'request')) return json(res, { error: 'forbidden' }, 403);
      ...
      const rows = await sql`
        ...
        values (${invoice.id},${payments[0]?.id || null},${invoice.booking_id},${invoice.patient_id},null,${auth.staff.id},now(),${amount},${String(body.reason_code || 'other')},${reason},'pending','pending','pending',jsonb_build_object('source','appwrite-neon-refund-workflow'))
        returning *`;
    ...
    if (action === 'approve_refund_doctor') {
      if (!allowed(auth.role, 'doctor')) return json(res, { error: 'forbidden' }, 403);
      const rows = await sql`update public.clinic_refund_requests set doctor_approval_status='approved', doctor_approved_by=${auth.staff.id}, doctor_approved_at=now(), doctor_approval_note=${String(body.note || '')}, status='pending' where id=${id} and status='pending' and doctor_approval_status='pending' returning *`;
    ...
    if (action === 'approve_refund_management') {
      if (!allowed(auth.role, 'management')) return json(res, { error: 'forbidden' }, 403);
      const rows = await sql`update public.clinic_refund_requests set management_approval_status='approved', management_approved_by=${auth.staff.id}, management_approved_at=now(), management_approval_note=${String(body.note || '')}, status='approved' where id=${id} and status='pending' and doctor_approval_status='approved' and management_approval_status='pending' returning *`;
      return rows[0] ? json(res, { provider: 'appwrite-neon', refund: rows[0] }) : json(res, { error: 'invalid_refund_state' }, 409);
    }
    if (action === 'process_refund') {
      if (!allowed(auth.role, 'process')) return json(res, { error: 'forbidden' }, 403);
      ...
      const rows = await sql`update public.clinic_refund_requests set status='processed', processed_by=${auth.staff.id}, processed_at=now(), refund_method=${method}, refund_reference=${reference || null} where id=${id} and status='approved' and doctor_approval_status='approved' and management_approval_status='approved' returning *`;
      return rows[0] ? json(res, { provider: 'appwrite-neon', refund: rows[0] }) : json(res, { error: 'invalid_refund_state' }, 409);
    }
    return json(res, { error: 'unknown_action' }, 400);
  } catch (error) {
    console.error('refund boundary failure', { name: error?.name, message: error?.message });
    return json(res, { error: 'refund_workflow_unavailable' }, 503);
  }
}

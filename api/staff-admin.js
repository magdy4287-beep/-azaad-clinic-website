...
async function authorize(request) {
  ...
  const accountResponse = await appwriteRequest('/account', { headers: { 'X-Appwrite-Session': secret } });
...
async function createUser(staff, sql) {
  ...
  const userId = 'unique()';
  const response = await appwriteRequest('/users', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ userId, email, password, name: String(staff.full_name || '').trim() || email }),
  });
  if (!response.ok) {
  ...
  try {
    const rows = await sql`
      insert into public.clinic_staff (auth_user_id, full_name, username, email, phone, role, active)
      values (${user.$id}, ${String(staff.full_name || '').trim()}, ${String(staff.username || '').trim().toLowerCase()}, ${email}, ${String(staff.phone || '').trim() || null}, ${role}, true)
      returning id, auth_user_id, full_name, username, email, phone, role, active
    `;
  ...
  try {
    ...
    if (action === 'list') {
      const rows = await sql`
        select id, auth_user_id, full_name, username, email, phone, role, active
        from public.clinic_staff
        order by lower(coalesce(full_name, username, email, '')) asc
      `;
    ...
    if (action === 'update_role') {
      ...
      if (!role) return json({ error: 'invalid_role' }, 400);
      const rows = await sql`
        update public.clinic_staff set role = ${role} where id::text = ${staffId} returning id, auth_user_id, full_name, username, email, phone, role, active
      `;
    ...
    if (action === 'enable' || action === 'disable') {
      ...
      const rows = await sql`
        update public.clinic_staff set active = ${active} where id::text = ${staffId} returning id, auth_user_id, full_name, username, email, phone, role, active
      `;
      if (!rows[0]) return json({ error: 'staff_not_found' }, 404);
      return json({ staff: rows[0], provider: 'appwrite-neon' });
    }
    ...
    if (action === 'reset_password') {
      ...
      const response = await appwriteRequest(`/users/${encodeURIComponent(authUserId)}/password`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },

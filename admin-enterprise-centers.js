...
(() => {
  ...
  const ENTERPRISE_BACKENDS = {
    patient360: 'azaad-patient-360', rcm: 'azaad-invoice-center',
    analytics: 'azaad-management-dashboard', finance: 'azaad-finance',
    marketing: 'azaad-management-dashboard', insights: 'azaad-ai-insights', security: 'azaad-security-center',
  };
  const ROLE_SCOPES = {
  ...
  const ensure = (key) => {
    ...
    panel.id = id; panel.className = 'panel';
  ...
  function bind() {
    ...
    Object.keys(D).filter(canAccess).forEach((key) => {
      ...
      if (button && !button.dataset.enterpriseBound) { button.dataset.enterpriseBound = '1'; button.addEventListener('click', () => render(key)); }
    });
  }
    ...
    try {
      ...
      if (key==='patient360') {
        ...
        $('patient360Search').onclick = async () => {
          const query = $('patient360Query').value.trim(); const output = $('patient360Result'); if (!query) return;
          output.innerHTML = '<div class="empty">⏳</div>';
          try { const data = await call(`/api/patient-financial-summary?q=${encodeURIComponent(query)}`); output.innerHTML = cards([['المريض', data.patient?.patient_name], ['MRN', data.patient?.mrn], ['الهاتف', data.patient?.patient_phone], ['الفواتير', money(data.summary?.total_invoices)], ['المدفوع', money(data.summary?.total_paid)], ['المتبقي', money(data.summary?.balance)]]); }
      ...
      if (key==='analytics') {
        const data = await call(`/api/admin-appointments?from=${today}&to=${today}`); const k = data.kpis || data.summary || {};
        body.innerHTML = cards([['الحجوزات', k.bookings || data.count], ['مؤكد', k.confirmed], ['مكتمل', k.completed], ['No-Show', k.no_show], ['معدل الإكمال', `${k.completion_rate || 0}%`]]); return;
      }
      if (key==='insights') {
        const data = await call(`/api/ai-insights?from=${today}&to=${today}`); const rows = data.insights || [];
      ...
      if (key==='rcm') { body.innerHTML = rcmRows([]); return; }
  ...
  const activateKeyFromDom = () => {
    const active = document.querySelector('.panel.active[id$="EnterprisePanel"]'); if (!active) return;
    const key = active.id.replace(/EnterprisePanel$/, ''); if (D[key] && canAccess(key)) render(key);
  };
  const handlePanelSignal = (event) => {
    const panel = event.detail?.panel || ''; const key = panel.endsWith('EnterprisePanel') ? panel.replace('EnterprisePanel', '') : '';
  ...
  const observeRoleReady = () => {
    ...
    roleObserver = new MutationObserver(() => { if (!role()) return; roleObserver.disconnect(); roleObserver = null; handleRoleReady(); });
  ...
  bind(); observeRoleReady(); queueMicrotask(activateKeyFromDom);

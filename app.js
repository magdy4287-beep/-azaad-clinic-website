...
return doctor.name || doctor.full_name || 'غير محدد';
...
return service?.name || service?.title || 'غير محدد';
...
try {
  ...
  /*
   * لا نقوم بعمل reset قبل إنشاء
   * رسالة WhatsApp حتى نضمن وجود
   * كل بيانات المريض في الرسالة.
   */
  const form =
    $('bookingForm');
  if (form) {

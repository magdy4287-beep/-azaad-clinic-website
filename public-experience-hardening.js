 ...
 * Safe UX/data-owner layer. Intentionally does not own translations,
 * authentication, booking payloads, or clinic data contracts.
 */
(() => {
  'use strict';

  const CLINIC_API = 'https://derofsthjivlkcdnojww.supabase.co/functions/v1/azaad-public-clinic-data';
  const STATE = '__AZAAD_PUBLIC_EXPERIENCE_HARDENING_V1__';
  ...
  function renderRecoveredPosts(posts) {
    ...
    grid.innerHTML = clean.map(p => {
      const title = p?.title || p?.title_en || 'Azaad';
      const content = p?.content || p?.content_en || '';
  ...
  async function recoverPostsIfNeeded() {
    const existing = window.__AZAAD_CLINIC_POSTS_V9__;
    if (existing?.posts?.length) {
      renderRecoveredPosts(existing.posts);
      return;
    }
    if (document.getElementById('clinicPostsGrid')?.children.length) return;
    try {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 8000);
      const response = await fetch(CLINIC_API, { cache:'no-store', signal:controller.signal, headers:{Accept:'application/json'} });
      clearTimeout(timer);

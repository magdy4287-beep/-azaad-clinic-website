(() => {
  ...
  function render(data){
    ...
    if(!document.getElementById('azaadTeamPublicStyles')){const s=document.createElement('style');s.id='azaadTeamPublicStyles';s.textContent='.azaad-team-grid{grid-template-columns:repeat(auto-fill,minmax(240px,1fr))}.azaad-team-photo{min-height:260px;height:auto;aspect-ratio:auto;border-radius:18px;overflow:visible;background:#eef1f7;display:flex;align-items:center;justify-content:center;font-size:64px;font-weight:900;color:#17214f}.azaad-team-photo img{width:auto;height:auto;max-width:100%;max-height:none;object-fit:contain !important;display:block}.azaad-team-photo:has(img){min-height:0}.azaad-team-photo span{display:flex;align-items:center;justify-content:center;width:100%;min-height:260px}';document.head.appendChild(s)}
  }

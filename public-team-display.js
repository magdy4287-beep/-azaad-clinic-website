(() => {
  ...
  function render(data){
    ...
    if(!document.getElementById('azaadTeamPublicStyles')){const s=document.createElement('style');s.id='azaadTeamPublicStyles';s.textContent='.azaad-team-grid{grid-template-columns:repeat(auto-fill,minmax(240px,1fr))}.azaad-team-photo{min-height:260px;height:auto;aspect-ratio:4/5;border-radius:18px;overflow:hidden;background:#eef1f7;display:flex;align-items:center;justify-content:center;font-size:64px;font-weight:900;color:#17214f}.azaad-team-photo img{width:100%;height:auto;max-height:none;object-fit:contain;display:block}';document.head.appendChild(s)}
  }

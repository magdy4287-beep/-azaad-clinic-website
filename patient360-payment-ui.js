...
(() => {
  ...
  function parseInvoice(row){
    ...
    const m=text.match(/(INV-[A-Z0-9-]+)\s*[•·]\s*([0-9]+(?:\.[0-9]+)?)\s*[•·]\s*unpaid/i);
    if(!m) return null;
    const id=row.dataset.invoiceId || row.getAttribute('data-invoice-id');
    return {number:m[1],amount:Number(m[2]),id};
  }
  function inject(){
    if(!allowed()) return;
    document.querySelectorAll('.p360-row').forEach(row=>{
      if(row.querySelector('[data-p360-payment]')) return;
      const inv=parseInvoice(row); if(!inv || !inv.id) return;
  ...
  async function pay(inv,host){
    ...
    if(!t || !inv.id){ host.insertAdjacentHTML('beforeend','<span class="p360-pay-error">تعذر تحديد الفاتورة أو الجلسة.</span>'); return; }
    btn.disabled=true; btn.textContent='⏳ جاري تسجيل الدفع...';
    try{
      const r=await fetch(PAYMENT_API,{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json',Authorization:`Bearer ${t}`,apikey:SUPABASE_KEY},body:JSON.stringify({p_invoice_id:inv.id,p_amount:inv.amount,p_method:method,p_reference:null,p_notes:'Patient 360 front-desk payment'})});
      const b=await r.json().catch(()=>({})); if(!r.ok) throw new Error(b?.message||b?.hint||b?.details||b?.error||`HTTP ${r.status}`);
      host.innerHTML='<span class="p360-paid">✅ Paid — '+esc(inv.amount.toFixed(2))+' EGP</span>';
      window.dispatchEvent(new CustomEvent('azaad:patient360-payment-complete',{detail:{invoiceId:inv.id,invoiceNumber:inv.number,amount:inv.amount,method}}));
    }catch(e){ btn.disabled=false; btn.textContent=`💳 تسجيل الدفع ${inv.amount.toFixed(2)} EGP`; host.insertAdjacentHTML('beforeend',`<span class="p360-pay-error">تعذر تسجيل الدفع: ${esc(e.message)}</span>`); }
  }

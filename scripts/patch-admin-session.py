...
# Replace only the legacy restoreStaff() implementation. Keep the rest of admin.html byte-for-byte unchanged.
...
# Make SIGNED_IN passive: the explicit restore() path owns startup restoration.
old = '''      const valid =\n        await restoreStaff();\n\n      if(\n        valid\n      ){\n        await load();\n      }'''

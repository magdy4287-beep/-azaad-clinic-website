...
for cmd in pg_dump pg_restore psql sha256sum openssl cmp; do command -v "$cmd" >/dev/null || { echo "ERROR: required command not found: $cmd" >&2; exit 127; }; done
...
SQL
# Neon provides public already; preserve it and omit only the archive's CREATE SCHEMA public entry.
pg_restore --exit-on-error --no-owner --no-privileges --section=pre-data --exclude-schema=public --dbname="$NEON_DATABASE_URL" "$work/public.restore.dump"
...
psql "$NEON_DATABASE_URL" -v ON_ERROR_STOP=1 <<'SQL'
CREATE TABLE IF NOT EXISTS public.azaad_dr_reconciliation (checked_at timestamptz NOT NULL DEFAULT now(), table_name text NOT NULL, row_count bigint NOT NULL, PRIMARY KEY (checked_at, table_name));
...
echo 'PASS: encrypted dump integrity'
echo 'PASS: Neon public schema preserved'
echo 'PASS: pre-data restore'
echo 'PASS: data restore'
echo 'PASS: identity-reference placeholders'
echo 'PASS: post-data restore'
echo 'PASS: reconciliation recorded'

...
async function main(){
 const rl=await req(`${endpoint}/functions/runtimes`,{headers}), rs=Array.isArray(rl.runtimes)?rl.runtimes:[];
 const deno=rs.find(x=>[x.key,x.slug,x.name,x.version,x.$id].filter(Boolean).some(v=>String(v).toLowerCase().startsWith('deno-')||String(v).toLowerCase()==='deno'));
 if(!deno) throw new Error(`FAIL-CLOSED: Appwrite Deno runtime is not available; active=${rs.map(x=>x.key||x.slug||x.name||x.version||x.$id).join(',')}`);
 const runtimeSlug=String(deno.key||deno.slug||deno.version||deno.$id);
 ...
 for(const name of source){const dir=path.join(root,name),entry=fs.existsSync(path.join(dir,'index.ts'))?'index.ts':fs.existsSync(path.join(dir,'index.js'))?'index.js':null;if(!entry)throw new Error(`FAIL-CLOSED: ${name} has no index.ts/index.js`);let fn=byId.get(name);
  ...
  const archive=path.join(process.env.RUNNER_TEMP,`${name}.tar.gz`);execFileSync('tar',['-czf',archive,'-C',dir,'.']);const sha=crypto.createHash('sha256').update(fs.readFileSync(archive)).digest('hex');

/* Azaad Clinic — Smart Booking Assistant
   Privacy rule: only scheduling data is sent to an AI provider; never send patient names,
   diagnoses, assessment answers, notes, or other clinical content.
   If an AI endpoint is not configured, the assistant falls back to deterministic ranking.
*/
(() => {
  'use strict';
  ...
  async function call(action,body){const r=await fetch(`${API}?action=${encodeURIComponent(action)}`,{method:'POST',headers:{Authorization:`Bearer ${token()}`,apikey:KEY,'Content-Type':'application/json'},body:JSON.stringify(body||{})});const b=await r.json().catch(()=>({}));if(!r.ok)throw Error(b.error||`HTTP ${r.status}`);return b}
  ...
  async function suggest(input){
    ...
    try{
      const r=await call('ai-booking-helper',{slots:slots.slice(0,80),preferred_date:input?.preferred_date||null,preferred_time:input?.preferred_time||null,reason:input?.reason||'followup'});
      if(Array.isArray(r.suggestions)&&r.suggestions.length)return r;
    }catch(_){/* provider unavailable: safe local ranking */}
    const ranked=rank(slots,input?.preferred_date||'',input?.preferred_time||'').slice(0,5);
    return {suggestions:ranked,message:'تم ترتيب المواعيد المتاحة ذكيًا حسب التاريخ والوقت المفضلين.'};
  }
  function mount(host){if(!host||host.querySelector('.azaadSmartBooking'))return;const box=document.createElement('section');box.className='azaadSmartBooking card';box.innerHTML=`<div class="panel-head"><div><h3>🤖 مساعد حجز الموعد الذكي</h3><div class="muted">يستخدم بيانات المواعيد فقط، ولا يرسل أي بيانات طبية أو معلومات تعريفية للمريض.</div></div></div><div class="azaad-booking-legend"><span class="azaad-legend-available">🟡 متاح</span><span class="azaad-legend-booked">🟢 محجوز</span><span class="azaad-legend-completed">🔴 منتهٍ</span><span class="azaad-legend-progress">🔵 جاري</span><span class="azaad-legend-pending">🟣 انتظار</span></div><div style="display:grid;grid-template-columns:1fr 1fr auto;gap:10px"><input class="azaadSmartDate" type="date"><input class="azaadSmartTime" type="time"><button type="button" class="btn btn-secondary azaadSmartSuggest">🤖 اقترح الأفضل</button></div><div class="azaadSmartResults" style="margin-top:12px"></div>`;host.appendChild(box);box.querySelector('.azaadSmartSuggest').onclick=async()=>{const date=box.querySelector('.azaadSmartDate').value,time=box.querySelector('.azaadSmartTime').value,res=box.querySelector('.azaadSmartResults');res.textContent='🤖 جاري ترتيب المواعيد...';try{const r=await suggest({preferred_date:date,preferred_time:time});res.innerHTML=r.suggestions.map(s=>`<button type="button" class="btn btn-secondary" style="margin:4px" data-smart-date="${esc(s.date)}" data-smart-time="${esc(s.time)}">🟡 ${esc(s.date)} — ${esc(s.time)}</button>`).join('')||`<div class="muted">${esc(r.message||'لا توجد اقتراحات.')}</div>`;res.querySelectorAll('[data-smart-date]').forEach(b=>b.onclick=()=>{box.querySelector('.azaadSmartDate').value=b.dataset.smartDate;box.querySelector('.azaadSmartTime').value=b.dataset.smartTime;});}catch(e){res.textContent='❌ '+e.message}}}

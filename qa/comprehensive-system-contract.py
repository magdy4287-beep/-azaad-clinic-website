...
require('data-role' in read(ROOT/'admin.js'),'admin does not expose authenticated role to UI shell');require("'OWNER','ADMIN','MANAGER','CASHIER'" in ft.replace(' ','') or 'OWNER','ADMIN' in ft,'RCM finance role scope missing')
...
for p,l in ((ROOT/'change-password.html','password page'),):require(p.exists(),f'{l} missing')

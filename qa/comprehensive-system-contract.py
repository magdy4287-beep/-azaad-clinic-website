...
normalized=re.sub(r'\s+',' ',rt).strip(); state_source=('const authenticatedState = window.AZAAD && window.AZAAD.state' in normalized) or bool(re.search(r'(?:const|let|var)\s+authenticatedState\s*=\s*window(?:\.AZAAD|\[["\']AZAAD["\']\])\s*\.state',normalized)); role_source=bool(re.search(r'(?:const|let|var)\s+authenticatedRole\s*=\s*authenticatedState(?:\?\.)?\.role',normalized)); role_projection=all(re.search(p,normalized) for p in (r'document\.body\.dataset\.role\s*=\s*current',r'document\.documentElement\.dataset\.role\s*=\s*current'))

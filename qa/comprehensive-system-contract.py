...
def read(p):
    try:
        ...
        FAILURES.append(f"cannot read {p}: {e}")
        return ""
...
build_runner = ROOT / "qa/vercel-build.py"
responsive = ROOT / "azaad-responsive-shell.css"
role_ui = ROOT / "azaad-role-experience.js"
injector = ROOT / "qa/inject-responsive-shell.py"
...
for p, label in (
    (central, "central-i18n.js"),
    (build_runner, "qa/vercel-build.py"),
    (responsive, "azaad-responsive-shell.css"),
    (role_ui, "azaad-role-experience.js"),
    (injector, "qa/inject-responsive-shell.py"),
    (enterprise, "admin-enterprise-centers.js"),
    (invoice_api, "api/invoices.js"),
    (staff_api, "api/staff-admin.js"),
...
ct = read(central)
vt = read(vercel)
bt = read(build_runner)
rt = read(role_ui)
et = read(enterprise)
it = read(invoice_api)
st_api = read(staff_api)

for token, msg in (
    ("window.AZAAD_I18N", "central I18N runtime API"),
    ("MutationObserver", "central I18N dynamic observer"),
    ("azaadLanguageChanged", "central language-change event"),
):
...
for step in (
    "qa/inject-central-i18n.py",
    "qa/inject-responsive-shell.py",
    ".github/patch-admin.py",
    "qa/remove-legacy-admin-i18n-runtime.py",
    ".github/inject-patient-actions.py",
    ".github/inject-doctor-actions.py",
    "qa/lazy-admin-modules.py",
    "qa/verify-production-contracts.py",
...
for r in ("OWNER", "ADMIN", "MANAGER", "SECRETARY", "RECEPTION", "CASHIER", "MARKETING"):
    require(r in rt, f"role navigation contract missing {r}")
normalized = re.sub(r"\s+", " ", rt).strip()
state_source = (
    "const authenticatedState = window.AZAAD && window.AZAAD.state" in normalized
    or bool(re.search(r"(?:const|let|var)\s+authenticatedState\s*=\s*window(?:\.AZAAD|\[[\"']AZAAD[\"']\])\s*\.state", normalized))
)
role_source = bool(re.search(r"(?:const|let|var)\s+authenticatedRole\s*=\s*authenticatedState\s*(?:&&\s*authenticatedState)?\s*(?:\?\.|\.)\s*role", normalized))
role_projection = all(
...
for h in htmls:
    ...
    if ".git/" in rel:
        continue
    if "central-i18n.js" not in t and "qa/inject-central-i18n.py" not in bt:
        FAILURES.append(f"{rel}: no central I18N runtime or build injection")
    if re.search(r"(?:lang|language)[^\n]{0,180}location\.reload\s*\(", t, re.I):
        FAILURES.append(f"{rel}: language switching contains location.reload()")

refund = ROOT / "refund-workflow-ui.js"
rf = read(refund) if refund.exists() else ""
require(refund.exists(), "refund workflow missing")
for x in ("approve_refund_doctor", "approve_refund_management", "process_refund", "doctor_approval_status", "management_approval_status"):
    require(x in rf, f"refund control missing: {x}")
...
for x, m in (("CANNOT_DISABLE_SELF", "self-disable protection"), ("LAST_OWNER_PROTECTED", "last-owner protection"), ("PASSWORD_UPDATE_FAILED", "password recovery")):
    require(x in st_api or x in st, f"{m} missing")
for x, m in (("suspend", "staff suspend capability"), ("disable", "staff disable capability"), ("reactivate", "staff reactivate capability")):
    require(x in st_api.lower() or x in st.lower(), f"{m} missing")

# AI governance is owned by the dedicated AI operating-system gate. This contract
# only proves that the canonical gate exists; it must never encode a retired provider path.
require(ai_gate.exists(), "AI operating-system gate missing")
...
names = {p.name for p in wd.glob("*.yml")} if wd.exists() else set()
for x in (
    "azaad-ai-gate.yml",
    ...
    require(x in names, f"missing workflow gate: {x}")
...
if len(admins) > 1:
    require("/admin/admin/:path*" in vt, "duplicate admin trees lack redirects")

for p in [p for p in ROOT.rglob("*.js") if ".git" not in p.parts]:
    t = read(p)
    rel = p.relative_to(ROOT).as_posix()
    if any(x in t for x in ("openai.com", "anthropic.com", "gemini.google.com")) and "qa/" not in rel:
        WARNINGS.append(f"{rel}: review external AI provider for Free-only compliance")

print("AZAAD comprehensive system contract")
print(f"HTML pages scanned: {len(htmls)}")
print(f"JS sources scanned: {len([p for p in ROOT.rglob('*.js') if '.git' not in p.parts])}")
print(f"Security/AI source files scanned: {len(sec)}")
if WARNINGS:
    print("WARNINGS:")
    [print(f"  - {x}") for x in sorted(set(WARNINGS))]
if FAILURES:
    print("FAILURES:")

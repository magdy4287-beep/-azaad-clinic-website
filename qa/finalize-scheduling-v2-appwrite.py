...
CANONICAL_MARKER = "/* AZAAD CLINIC — Scheduling V2 canonical Appwrite-Neon UI */"
if CANONICAL_MARKER in text:
    # The production build must be idempotent. A source file already migrated to
    # the canonical owner is valid input and must not be rewritten or rejected.
    if any(marker in text for marker in (
        ...
        raise SystemExit('FAIL-CLOSED: canonical scheduling-v2 source still contains a retired provider marker')
    print('PASS: Scheduling V2 source is already canonical Appwrite session + consolidated Neon API; no source rewrite required')
    raise SystemExit(0)

if text.count("const client = () => window.AZAAD?.supabase || null;") != 1:
    raise SystemExit('FAIL-CLOSED: scheduling-v2 legacy client marker count != 1')
if text.count("async function db(table,columns='*',filter)") != 1:
    raise SystemExit('FAIL-CLOSED: scheduling-v2 db marker count != 1')
if text.count("async function invoke(action,body)") != 1:
    raise SystemExit('FAIL-CLOSED: scheduling-v2 invoke marker count != 1')
...
"""
if old not in text:
    raise SystemExit('FAIL-CLOSED: expected legacy scheduling helper block not found')
...
"""
if old_patient not in text:
    raise SystemExit('FAIL-CLOSED: expected legacy patient search block not found')
...
if any(x in text for x in (
    ...
    raise SystemExit('FAIL-CLOSED: Supabase runtime marker survived scheduling-v2 transform')

...
Diagnostic only: it blocks architectural drift and names the canonical owner
that must be repaired. It never mutates source code automatically.
"""
...
def main():
    ...
    for path in runtime_files():
        ...
        if "api" not in relative.parts and re.search(r"(?:access_token|refresh_token)\b", text, re.I):
            failures.append(
                f"{relative}: browser/runtime token state detected; "

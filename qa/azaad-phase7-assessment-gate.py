...
files = [p for p in ROOT.rglob('*') if p.is_file() and p.suffix in {'.html','.js','.ts','.tsx','.sql'} and 'node_modules' not in p.parts and '.git' not in p.parts]
...
checks = {
 ...
 'no AI patient identifiers': 'no patient identifiers' in lower or 'patient identifiers' in lower,
}
for name, ok in checks.items(): print(f"{'PASS' if ok else 'FAIL'}: {name}")
failed=[k for k,v in checks.items() if not v]
if failed: raise SystemExit('Phase 7 Assessment Gate failed: '+', '.join(failed))

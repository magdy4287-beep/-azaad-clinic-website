...
appointments_cookie_guard = (bool(re.search(r'const\s+cookie\s*=\s*`a_session_\$\{project\}=\$\{secret\};\s*a_session_\$\{project\}_legacy=\$\{secret\}`\s*;', appointments)) and bool(re.search(r'\bCookie\s*:\s*cookie\b', appointments)) and bool(re.search(r'\bappwriteAccount\(secret\)', appointments)))
appointments_role_guard = (bool(re.search(r"\b(?:STAFF_ROLES|role)\b", appointments)) and all(role in appointments for role in ('OWNER', 'ADMIN', 'MANAGER', 'SECRETARY', 'RECEPTION', 'DOCTOR')) and bool(re.search(r'\bSTAFF_ROLES\.(?:has|includes)|\b(?:includes|has)\(role\)', appointments)))
no_custom_header_guard = 'x-azaad-appwrite-session' not in auth.lower() and 'x-azaad-appwrite-session' not in appointments.lower()
canonical_admin_source_guard = (
    'async function restoreStaffProfile()' in admin
    and "fetch('/api/admin-auth'" in admin
    and "credentials: 'include'" in admin

...
global_restore = restore_source.replace('async function restoreStaffProfile()', 'async function restoreStaffProfile()', 1)
text = text.rstrip() + '\n\n/* Canonical global Appwrite restore owner: deliberately appended after all legacy scopes. */\nwindow.AZAAD_RESTORE_STAFF_PROFILE = ' + global_restore + ';\n'
...
for name, value in [('admin.js', text), ('admin.html', html_text)]:
    if 'functions/v1/staff-admin' in value: raise SystemExit(f'Legacy Supabase staff-admin endpoint remains in {name}')

...
# Retire browser-side Supabase client/import/configuration. The repository retains
# Supabase only as migration/evidence; this transform makes the Admin artifact
# strictly Appwrite-cookie + same-origin API.
...
# Remove legacy auth-state listeners / global Supabase exposure when present.
text = re.sub(r'/\* ============================================================\n\s*AUTH STATE\n\s*============================================================ \*/[\s\S]*?supabase\.auth\.onAuthStateChange\([\s\S]*?\n\);\s*\n?', '\n', text, count=1)
text = re.sub(r'\bwindow\.AZAAD\s*=\s*\{\s*supabase,\s*', 'window.AZAAD = {\n  ', text, count=1)

# The old controller had browser access-token session operations. Remove only
# the known auth helper bodies; feature modules continue to consume state/exports.
text = re.sub(r'/\* ============================================================\n\s*RESTORE SESSION\n\s*============================================================ \*/[\s\S]*?(?=/\* ============================================================\n\s*RESTORE STAFF)', '\n', text, count=1)
...
for pattern in (
    ...
    if re.search(pattern, text, flags=re.I):
        raise SystemExit(f"Legacy Supabase browser auth marker remains: {pattern}")
...
print("finalize-appwrite-admin-auth.py: cookie-only Appwrite browser boundary established")

...
replacement = '''async function restoreStaffProfile() {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      // Never sign out during startup restoration. A transient boundary/network failure
      // must not destroy a valid persisted Supabase session.
...
required = [
    ...
    "response.status === 403",
    "for (let attempt = 1; attempt <= 3; attempt += 1)",
    "Never sign out during startup restoration",
]
...
match = bounds(js, "async function restoreStaffProfile()")
body = js[match[0]:match[1]] if match else ""

...
(function () {
  ...
  async function session() {
    ...
    state.role = String(data.staff?.role || role()).toUpperCase();
    return data;
  }
  ...
  function renderShell() {
    ...
    box.id = 'staffManagementCenter';
    box.className = 'card';
    box.dir = 'rtl';
    box.innerHTML = `
      <div class="panel-head"><div><h2>👥 إدارة الموظفين</h2><div class="muted">Appwrite Identity + Neon Staff API</div></div><button id="staffAddButton" class="btn btn-primary" type="button">➕ إضافة موظف</button></div>
      <div id="staffManagementStats" class="stats" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));margin:15px 0"></div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin:15px 0"><input id="staffSearch" type="search" autocomplete="off" placeholder="🔎 الاسم / البريد / الهاتف" style="flex:1;min-width:240px"><select id="staffRoleFilter" style="min-width:180px"><option value="">كل الوظائف</option>${ROLES.map(r=>`<option value="${r}">${r}</option>`).join('')}</select></div>
      <div class="table-wrap"><table style="min-width:760px"><thead><tr><th>الاسم</th><th>البريد</th><th>الوظيفة</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody id="staffTableBody"></tbody></table></div>
      <div id="staffMessage" class="muted" style="margin-top:12px"></div>`;
  ...
  function renderRows() {
    ...
    const rows = state.staff.filter(s => {
      const hay = [s.full_name,s.name,s.email,s.phone,s.username,s.role].map(v=>String(v||'').toLowerCase()).join(' ');
      return (!q || hay.includes(q)) && (!filter || String(s.role||'').toUpperCase() === filter);
    });
    body.innerHTML = rows.length ? rows.map(s => {
      const active = s.active !== false;
      const id = esc(s.id || s.auth_user_id || '');
  ...
  async function load() {
    ...
    try {
      ...
      state.staff = Array.isArray(data.staff) ? data.staff : Array.isArray(data) ? data : [];
      const active = state.staff.filter(s=>s.active!==false).length;
      const stats = document.getElementById('staffManagementStats');
      if (stats) stats.innerHTML = `<div class="stat"><div class="stat-number">${state.staff.length}</div><div class="muted">إجمالي الموظفين</div></div><div class="stat"><div class="stat-number">${active}</div><div class="muted">حسابات نشطة</div></div><div class="stat"><div class="stat-number">${state.staff.length-active}</div><div class="muted">حسابات موقوفة</div></div>`;
      renderRows();
  ...
  async function createStaff() {
    if (!['OWNER','ADMIN','MANAGER'].includes(role())) { show('غير مصرح بإضافة موظف.'); return; }
    const full_name = window.prompt('اسم الموظف'); if (!full_name) return;
    const email = window.prompt('البريد الإلكتروني'); if (!email) return;
    const password = window.prompt('كلمة المرور المؤقتة'); if (!password) return;
    const selected = window.prompt(`الوظيفة (${ROLES.join(', ')})`, 'RECEPTION');
    const staffRole = ROLES.includes(String(selected||'').toUpperCase()) ? String(selected).toUpperCase() : 'RECEPTION';
    try { await api('create', { full_name, email, password, role: staffRole }); await load(); }
    catch (error) { show(error.message || 'تعذر إنشاء الموظف'); }
  }

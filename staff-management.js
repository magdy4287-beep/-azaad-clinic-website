...
* Staff administration uses authenticated JWT only.
    ============================================================ */
...
/*
Preferred Azaad Clinic project client.
*/
...
if (
  azaadClient &&
  azaadClient.auth &&
  typeof azaadClient.auth.getSession ===
    "function"
) {
  return azaadClient;
}
/*
  ...
  IMPORTANT:
  window.supabase may be the library namespace
  rather than the initialized client.
  Therefore we verify getSession().
*/
const globalClient =
  window.supabase;
if (
  globalClient &&
  globalClient.auth &&
  typeof globalClient.auth.getSession ===
    "function"
) {
  return globalClient;
}
...
modal.innerHTML = `
  ...
  >
    ...
    <div
      ...
      "
    >
      ⚠️ احفظ كلمة المرور الآن.
      <br>
      لن يتم تخزينها في واجهة الموظفين.
    </div>
    <div
      id="temporaryPasswordValue"
      dir="ltr"
      style="
        margin-top:18px;
        padding:16px;
  ...
  ?.addEventListener(
    ...
    async () => {
      try {
        ...
        showMessage(
          "⚠️ تعذر النسخ تلقائيًا. اضغط مطولًا على كلمة المرور لنسخها.",
          "error"
        );
      }
    }
...
/*
Wait because the main application is responsible
